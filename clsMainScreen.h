...
private :
	enum enMainMenuOptions {  ShowClientsList = 1,  AddNewClient = 2 , DeleteClient = 3, UpdateClient = 4
		                     , FindClient = 5, Transactions = 6, MangeUser = 7,eLoginRegister=8,eCurrencyScreen = 9, Exit = 10};
	...
	{
		cout << "\nPlease choose what do you want to do? [1 to 10] : ";
		return clsInputValidate::ReadIntNumberBetween(1, 10, "Enter Number between [1] and [10] again : ");
	}
	static enMainMenuOptions _PerformMainMenuOption(short MainMenuOption )
	{
		...
		case enMainMenuOptions::eLoginRegister :
			...
			_ShowLoginRegister();
			_GoBackToMainMenu();
		case enMainMenuOptions::eCurrencyScreen:
			system("cls");
			_ShowCurrencyScreen();
			_GoBackToMainMenu();
